...
type (
	// FindAllLocationsCommand is the command line data structure for the FindAll action of Locations
	FindAllLocationsCommand struct {
		Game        string
		PrettyPrint bool
	}
	...
	// PlayLocationsCommand is the command line data structure for the Play action of Locations
	PlayLocationsCommand struct {
		Payload     string
		ContentType string
		ID          int
		Lat         string
...
// Run makes the HTTP request corresponding to the FindAllLocationsCommand command.
func (cmd *FindAllLocationsCommand) Run(c *client.Client, args []string) error {
	var path string
	if len(args) > 0 {
		path = args[0]
	} else {
	...
	resp, err := c.FindAllLocations(ctx, path, stringFlagVal("game", cmd.Game))
	if err != nil {
...
// Run makes the HTTP request corresponding to the GetLocationsCommand command.
func (cmd *GetLocationsCommand) Run(c *client.Client, args []string) error {
	var path string
	if len(args) > 0 {
		path = args[0]
	} else {
...
func (cmd *PlayLocationsCommand) Run(c *client.Client, args []string) error {
	var path string
	if len(args) > 0 {
		path = args[0]
	} else {
